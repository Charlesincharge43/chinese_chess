...
export class Game {//Because it'd be a nightmare to hook everything up and shiz, I'm simply make this new state JUST for AI to simulate on
//NEED TO DEEP CLONE USING DEEPCLONE FUNCTIONS YOU HAD TO WASTE TIME WRITING... BECAUSE YOU WERE AN IDIOT AND USED THE HORRIBLY STRUCTURED CHESSSTATE FROM TUTORIAL, NOW IT'S BITING YOU IN THE ASS
  constructor(boardState, chessState, parent=null, turnNumber=0){
  ...
  display () {//a quick and easy way to display the board on the console, without having to render to canvas (for debugging purposes)
...
// }
  ...
  populateLegalMovesforTeam (team){
    let legalMoves= new LegalMoves({chessState: this.chessState, boardState: this.boardState});
    let arrToPop=[];
  ...
  createMove (piece,targetLocObj){//create movePieceObj for this.move/next_state/new_state_makeChildNode
    return { selectedPieceLookupVal: {key: piece.pieceKey, team: piece.team }, targetLoc: targetLocObj }
  }

  move (movePieceObj){//movePieceObj-> { selectedPieceLookupVal: {...}, targetLoc: {x:... , y:... } } //this will NOT change the turn after moving (need to invoke next_state to incorporate both movement and turn change)
    /*
...
// }
  ...
  evaluate (){//if it's simply too resource intensive for MCTS to simulate all the way to a win or a loss, this can be used to evaluate who is *winning* or *losing*
